describe('Scenario Routes: ', function() {
    beforeEach(module('reviewManagerApp.scenario'));
	...
	describe('Index route: ', function() {
		beforeEach(inject(
			function($httpBackend) {
				...
				$httpBackend.whenGET('/api/scenario').respond([]);
	...
	describe('Create route: ', function() {
		beforeEach(inject(
			function($httpBackend) {
				...
				$httpBackend.whenGET('/api/scenario/create').respond([]);
				$httpBackend.whenGET('/api/requirement').respond([]);
			}
		));
	...
	describe('Edit route: ', function() {
		beforeEach(inject(
			function($httpBackend) {
				$httpBackend.whenGET('create-edit.html').respond(200, 'list page');
				$httpBackend.whenGET('/api/scenario/1').respond([]);
				$httpBackend.whenGET('/api/requirement').respond([]);
			}
		));
