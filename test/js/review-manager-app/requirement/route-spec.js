describe('Requirement Routes: ', function() {
    beforeEach(module('reviewManagerApp.requirement'));
	...
	describe('Index route: ', function() {
		beforeEach(inject(
			function($httpBackend) {
				...
				$httpBackend.whenGET('/api/requirement').respond([]);
				$httpBackend.whenGET('/api/review').respond([]);
				$httpBackend.whenGET('/api/qualityGroup').respond([]);
			}
		));
	...
	describe('Create route: ', function() {
		beforeEach(inject(
			function($httpBackend) {
				...
				$httpBackend.whenGET('/api/requirement/create').respond([]);
				$httpBackend.whenGET('/api/review').respond([]);
				$httpBackend.whenGET('/api/qualityGroup').respond([]);
			}
		));
	...
	describe('Edit route: ', function() {
		beforeEach(inject(
			function($httpBackend) {
				$httpBackend.whenGET('create-edit.html').respond(200, 'list page');
				$httpBackend.whenGET('/api/requirement/1').respond([]);
				$httpBackend.whenGET('/api/review').respond([]);
				$httpBackend.whenGET('/api/qualityGroup').respond([]);
			}
		));
